...
export default function GroupsPage() {
  ...
  if (groups.isLoading || stats.isLoading) return <Skeleton rows={6} label="Loading table" />;

  if (groups.isError || stats.isError) {
    return <div role="alert" className="rounded border border-danger/30 bg-danger/10 p-4 text-danger">Не удалось загрузить группы.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <header>
        <p className="text-sm font-semibold uppercase tracking-wide text-purple">T-6 Groups</p>
            ...
            return (
              <Card key={group.id} className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Link className="focus-ring rounded border border-royal px-3 py-2 text-sm font-medium text-royal hover:bg-royal/5" to={`/teacher/analytics?group_id=${group.id}`}>
                    Список студентов
                  </Link>
                  <Link className="focus-ring rounded bg-purple px-3 py-2 text-sm font-medium text-white hover:bg-purple/90" to="/teacher/scenarios">
                    Назначить сценарий
                  </Link>
                </div>
              </Card>
