...
**Владелец фикса:** Claude Opus 4.7 (архитектурно), затем Codex GPT 5.5 (реализация `tokens.css` и `components/ui/`).

### C-02. Конфликт имён агентов между документами
**Где:** §20 (`Gemini 3.1 Pro`) vs `AGENTS.md`/`CLAUDE.md`/`.codex/*.toml` (`Codex GPT 5.5`).
...
**Исправление:** §20 ADDENDUM переписывает распределение под **Claude Opus 4.7 + Codex GPT 5.5** (как в стартере). §20 дизайн-документа помечен как outdated.
...
**Владелец:** Codex GPT 5.5 (реализация), Claude Opus 4.7 (ревью).
...
**Проблема:** Codex GPT 5.5 при реализации этих страниц будет импровизировать, получим несогласованный UI.
...
5. ✅ `docs/AGENT_TASKS.md` — детальное распределение задач между Claude Opus 4.7 и Codex GPT 5.5 для каждого из 10 этапов + общий список «кто что делает»
